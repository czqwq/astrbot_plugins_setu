...
| max_retries | integer | 最大重试次数，当API请求失败时的最大重试次数，默认为3次 |
...
- **白名单（group_whitelist）**：只有在白名单中的群聊才能使用插件功能（管理员除外）
- **黑名单（group_blacklist）**：黑名单中的群聊无法使用插件功能（管理员除外）
- **用户黑名单（user_blacklist）**：被列入黑名单的用户在任何地方都无法使用插件功能

注意：管理员账户不受黑白名单限制，可以在任意群聊中使用插件功能。

## 🔁 重试机制

插件内置了请求重试机制，当API请求失败时会自动重试，最大重试次数可通过[max_retries](file:///E:/IDEA/astrbot_plugin_setu/_conf_schema.json#L35-L41)配置项设置。每次重试都会在控制台输出日志，方便排查问题。
